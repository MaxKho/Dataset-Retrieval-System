...
UTILS = os.path.join(os.path.dirname(__file__), "utilities")
sys.path.insert(0, UTILS)

# 2) Bring in your downloader + constants
from download_inference_embeddings import download_and_unzip_from_gdrive, TARGET_DIR

download_and_unzip_from_gdrive()
DATA_DIR = TARGET_DIR
...
# … rest of demo_retrieval.py unchanged …
...
def run_partial(query_input: str, top_k: int = 10):
    ...
    with open(os.path.join(DATA_DIR, "doc_embeddings_partial.pkl"), "rb") as f:
        doc_embeddings = pickle.load(f)
    with open(os.path.join(DATA_DIR, "final_test.json"), "r", encoding="utf-8") as f:
        data = json.load(f)
...
def run_joint(query_input: str, top_k: int = 10):
    ...
    with open(os.path.join(DATA_DIR, "doc_embeddings_joint.pkl"), "rb") as f:
        doc_embeddings = pickle.load(f)
    with open(os.path.join(DATA_DIR, "final_test.json"), "r", encoding="utf-8") as f:
        data = json.load(f)
